...
impl FromStr for Threshold {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let [size, duration]: [&str; 2] = s
            .split(':')
            .collect::<Vec<_>>()
            .try_into()
            .map_err(|_| "invalid format (should be SIZE:DURATION_SEC)")?;
...
impl Determiner {
    ...
    pub fn determine(&self, size: u64) -> Option<Duration> {
        self.0.iter().find(|t| size <= t.size).map(|t| t.duration)
    }
}
...
mod tests {
    ...
    fn determine() {
        ...
        .unwrap();
        assert_eq!(
            determiner.determine(100 * 1024),
            Some(Duration::from_secs(24 * 60 * 60))
        );
        assert_eq!(
            determiner.determine(64 * 1024 * 1024),
            Some(Duration::from_secs(24 * 60 * 60))
        ...
        assert_eq!(
            determiner.determine(100 * 1024 * 1024),
